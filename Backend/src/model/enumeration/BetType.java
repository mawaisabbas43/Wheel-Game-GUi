...
public enum BetType
{
   RED
   {
      ...
      {
         if(winSlot.getColor().name().equals("RED"))//if red is wining slot
         {
        	 player.setPoints(player.getPoints()+player.getBet());//previous Points + bet
         }
         else
         {
        	 player.setPoints(player.getPoints()-player.getBet());//previous Points - bet
   ...
   Black
   {
	   @Override
	      public void applyWinLoss(Player player, Slot winSlot)
	      {
		   if(winSlot.getColor().name().equals("BLACK"))//if red is wining slot
	         {
	        	 player.setPoints(player.getPoints()+player.getBet());//previous Points + bet
	         }
	         else
   ...
   Zeros
   {
	   @Override
	      public void applyWinLoss(Player player, Slot winSlot)
	      {
		   if(winSlot.getColor().name().equals("GREEN0") || winSlot.getColor().name().equals("GREEN00"))//if red is wining slot
	         {
	        	 player.setPoints( player.getPoints() + ((Slot.WHEEL_SIZE/2)-1)*player.getBet());//previous Points + bet
	         }
	         else
	         {
	        	 player.setPoints(player.getPoints()-player.getBet());//previous Points - bet
